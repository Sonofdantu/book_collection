...
RSpec.describe Member, type: :model do
  # CREATE TESTING
  describe 'create_validations' do
  ...
  describe 'update_validations' do
    it 'is valid with valid attributes' do
    ...
    it 'is valid with hyphenated name' do
      member = Member.update(totalPoints: 10, full_name: 'Mary ann Doe', position: 'Treasurer')
    ...
    it 'is invalid with negative numbers for total points' do
      member = Member.update(totalPoints: 7, full_name: 'Mary-ann Doe', position: 'Treasurer')
      member.update(totalPoints: -7, full_name: 'Mary-ann Doe', position: 'Treasurer')
      expect(member).to_not be_valid
    end

    it 'is not valid without a name' do
      member = Member.update(totalPoints: 10, full_name: 'Joe Doe', position: 'Treasurer')
      member.update(totalPoints: 10, full_name: '', position: 'Treasurer')
      expect(member).to_not be_valid
    end

    it 'is not valid without position' do
      member = Member.update(totalPoints: 10, full_name: 'John Doe', position: 'GOAT')
      member.update(totalPoints: 10, full_name: 'John Doe', position: '')
      expect(member).to_not be_valid
    end

   it 'is not valid with numbers in name' do
      member = Member.update(totalPoints: 10, full_name: 'John Doe', position: 'Treasurer')
