class EventsController < ApplicationController
    ...
    respond_to do |format|
      format.html do
        redirect_to events_url,
                    notice: 'Event and associated attendances were successfully destroyed.'
      end
  ...
  def register_attendance
    ...
    if @event.password == params[:event_password]
      attendance = Attendance.where(email: current_admin.email,
                                    event_name: @event.title).first_or_initialize
